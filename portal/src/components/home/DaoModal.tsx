...
export const DaoModal: React.FC<DaoModalProps> = ({ account,signer,daojson, onClose }) => {
  ...
  const [vices,setDevices] = useState<Device[]>([]);
  ...
  for (let i = 0; i < daojson.devices.length; i++) {

    devices.push({
      dev_name: daojson.devices[i].device_name,
      endpoint: daojson.devices[i].endpoint
    });
  }
  ...
  useEffect(() => {
    const loadContracts = async () => {
      if (signer) {
        const dao = new ethers.Contract(daojson.governance_address, daojson.governance_abi, signer);
        const proposal_filter = dao.filters.ProposalCreated();
